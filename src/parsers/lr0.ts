...
export function parse(grammarStr: string, inputStr: string): ParseResult {
  ...
  for (const state of automaton.states) {
    ...
    // Separar items con dot al final vs en medio
    ...
    // Reduces — LR(0) reduce en TODOS los terminales + $
    for (const item of reduceItems) {
      if (item.head === grammar.augmentedStart) {
        // Accept
        const existing = actionTable.get(state.id)!.get('$');
        if (existing) conflicts.push(`Conflicto en estado ${state.id} con '$'`);
        else actionTable.get(state.id)!.set('$', { type: 'accept' });
        continue;
      }
