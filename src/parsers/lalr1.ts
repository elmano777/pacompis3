...
function mergeToLALR(
  ...
  const mergeGroups: LALRMergeGroup[] = [];
  for (const group of coreGroups.values()) {
    const sortedGroup = [...group].sort((a, b) => a - b);
    const rep = sortedGroup[0];
    for (const id of sortedGroup) mergeMap.set(id, rep);
    mergeGroups.push({ representative: rep, members: sortedGroup });
  }
...
export function compile(grammarStr: string): CompiledParser {
  try {
    ...
    const { states, transitions, mergeGroups } = mergeToLALR(lr1States, lr1Transitions);

    const automataData = buildAutomataData(states, transitions);
    const lr1Automata = buildAutomataData(lr1States, lr1Transitions);
    const lalrConstruction = {
      lr1Automata,
      lalrAutomata: automataData,
      mergeGroups: mergeGroups.filter((g) => g.members.length > 1),
    };
